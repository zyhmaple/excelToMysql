...
public class checkSheetStructure {
	...
	public static ExcelSheetObject checkSheet(XSSFSheet sheet) {
		...
		for (int i = 1; i <= rowNum; i++) {
			...
			int blackCellNum = 0;
			/*
			 * while(col.hasNext()&&threshold>0) { XSSFCell xcell = (XSSFCell)
			 * col.next(); if("".equals(getCellValue(xcell))) blackCellNum++; }
			 */
		...
		AnalysisColumnValue(checkResult);
		return checkResult;
	...
	public static void AnalysisColumnField(ExcelSheetObject checkResult) {
		...
		for (int i = colHeader.X - 1; i < colHeaderLth; i++) {
			...
			cfield.setColumnName(processColumnName(cell));
	...
	public static String getColumnFieldStr(ExcelSheetObject checkResult)
	{
		StringBuilder columnFields = new StringBuilder();
		for (ColumnField cf:checkResult.getColumnFileds()) {
			columnFields.append(cf.columnName).append(",");
		}
		return columnFields.substring(0,columnFields.length()-1).toLowerCase();
	}
	public static void AnalysisColumnValue(ExcelSheetObject checkResult) {
		Coordinate colHeader = checkResult.getLeftTop();
		int colHeaderLth = checkResult.getColumnNum();
		int rowValueLth = checkResult.getRowNum() - 1;
		List<String> rowValueList = new ArrayList<String>(rowValueLth);

		for (int j = colHeader.Y; j < rowValueLth; j++) {
			XSSFRow row = checkResult.getSheet().getRow(j);
			StringBuilder columnValues = new StringBuilder();
			for (int i = colHeader.X - 1; i < colHeaderLth; i++) {
				XSSFCell cell = row.getCell(i);
				columnValues.append(getCellValue(cell)).append(",");
			}
			rowValueList.add(columnValues.toString().substring(0, columnValues.length() - 1));
		}
		
		checkResult.setColumnValues(rowValueList);
	}
	...
	/*
	 * private static String getCellFormatValue(XSSFCell cell) { String
	 * cellvalue = ""; if (cell != null) // 判断当前Cell的Type switch
	 * (cell.getCellType()) { // 如果当前Cell的Type为NUMERIC case
	 * XSSFCell.CELL_TYPE_NUMERIC: case XSSFCell.CELL_TYPE_FORMULA: { //
	 * 判断当前的cell是否为Date if (HSSFDateUtil.isCellDateFormatted(cell)) { //
	 * 如果是Date类型则，取得该Cell的Date值 Date date = cell.getDateCellValue(); //
	 * 把Date转换成本地格式的字符串 cellvalue = cell.getDateCellValue().toLocaleString(); }
	 * // 如果是纯数字 else { // 取得当前Cell的数值 double num = new
	 * Double((double)cell.getNumericCellValue()); cellvalue =
	 * String.valueOf(myformat.format(num)); } break; } // 如果当前Cell的Type为STRIN
	 * case XSSFCell.CELL_TYPE_STRING: // 取得当前的Cell字符串 cellvalue =
	 * cell.getStringCellValue().replaceAll("'", "''"); break; // 默认的Cell值
	...
	public static void distinctRowColumnNum(ExcelSheetObject checkResult) {
		...
		int sumColumnCount = checkResult.getColumnNumOfEveryRow().size();
		...
		/*
		 * TreeMap<Integer,Integer> percentRate = new
		 * TreeMap<Integer,Integer>(); for(Entry<Integer, Integer> kv
		 * :columnNum.entrySet()){ percentRate.put(kv.getKey(),
		 * kv.getValue()/sumColumnCount); double rate =
		 * kv.getValue()/sumColumnCount; columnCount =
		 * kv.getKey()>columnCount?kv.getKey():columnCount; }
		 */
	...
	public static void findLeftTopAndRightBottom(ExcelSheetObject checkResult) {
		...
		for (Entry<Integer, Integer> kv : checkResult.getRowNumOfEveryColumn().entrySet()) {
			if (kv.getValue() >= checkResult.getRowNum()) {
				...
				rightx = (tempx > rightx) ? tempx
						: ((rightx - leftx + 1) < checkResult.getColumnNum() ? kv.getKey() : rightx);
